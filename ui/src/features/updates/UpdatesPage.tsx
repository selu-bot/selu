...
import { AlertTriangle, CheckCircle2, Cloud, Download, RefreshCw, RotateCcw, Settings2, ShieldCheck, Smartphone, UploadCloud } from 'lucide-react'
import { ApiError } from '../../api'
import { defineTranslations, useLanguage, useTranslations } from '../../i18n'
import { useNotices, useQueryErrorNotice } from '../../notices'
import { Button, ConfirmDialog, EmptyState, Field, Input, PageHeader, Select, StatusBadge } from '../../shared/ui'
...
export function UpdatesPage() {
  ...
  useQueryErrorNotice(forbidden ? null : settings.error ?? status.error, copy.loadError)
  ...
  const apply = useMutation({ mutationFn: updatesApi.apply, onSuccess: () => { setConfirming(null); notices.info(copy.started); void refresh() }, onError: (error) => notices.error(error, copy.applyFailed) })
  const rollback = useMutation({ mutationFn: updatesApi.rollback, onSuccess: () => { setConfirming(null); notices.info(copy.rollbackStarted); void refresh() }, onError: (error) => notices.error(error, copy.rollbackFailed) })
...
function UpdatesContent({ copy, language, settings, status, checking, onCheck, onApply, onRollback }: { copy: Copy; language: string; settings: UpdateSettings; status: UpdateStatus; checking: boolean; onCheck: () => void; onApply: () => void; onRollback: () => void }) {
  ...
  return <div className="final-stack">
    <OverviewGrid>
      ...
      <OverviewCard icon={<Cloud />} status={<StatusBadge tone="info">{settings.release_channel}</StatusBadge>} title={copy.settings} description={settings.public_origin || settings.current_origin} meta={`${copy.automatic}: ${settings.auto_update ? copy.enabled : copy.disabled}`} />
    </OverviewGrid>
    {(running || failed) && <ManagementSection title={running ? (status.progress_key.includes('rollback') ? copy.rollingBack : copy.updating) : copy.needsAttention} description={running ? copy.reconnecting : copy.serviceUnavailable}>
      <div className="final-section-stack"><div className="final-progress" role="progressbar" aria-valuemin={0} aria-valuemax={100} aria-valuenow={percent}><span style={{ width: `${percent}%` }} /></div><p className={failed ? 'final-callout is-danger' : 'final-callout'}>{progressText(copy, status.progress_key)}</p>{status.active_job_id && <small className="final-code">{copy.activeJob}: {status.active_job_id}</small>}</div>
    </ManagementSection>}
...
function SettingsSheet({ open, copy, settings, onClose, onSaved }: { open: boolean; copy: Copy; settings: UpdateSettings; onClose: () => void; onSaved: () => void }) {
  ...
  const [origin, setOrigin] = useState(settings.public_origin)
  ...
  useEffect(() => {
    if (open) {
      ...
      setOrigin(settings.public_origin)
  ...
  const save = useMutation({
    ...
    onSuccess: onSaved,
    onError: (error) => {
      if (error instanceof ApiError && error.code === 'invalid_public_origin') setValidationError(copy.invalidAddress)
      else notices.error(error, copy.saveFailed)
    },
  })
  const submit = () => {
    const nextOrigin = origin.trim()
    if (!isValidPublicOrigin(nextOrigin)) {
      setValidationError(copy.invalidAddress)
      return
    }
    setValidationError('')
    save.mutate({ release_channel: channel, public_origin: nextOrigin, auto_update: automatic, push_notifications_enabled: push, installation_telemetry_opt_out: !telemetry })
  }
  return <ManagementSheet open={open} title={copy.settings} description={copy.description} closeLabel={copy.close} onClose={onClose} busy={save.isPending} actions={<><Button onClick={onClose}>{copy.cancel}</Button><Button variant="primary" loading={save.isPending} onClick={submit}>{copy.save}</Button></>}>
    <div className="management-form">
      ...
      <Field label={copy.publicAddress} hint={<>{copy.publicAddressHint}<br />{copy.currentAddress.replace('{address}', settings.current_origin)}</>}><div className="final-inline-fields"><Input type="url" value={origin} onChange={(event) => { setOrigin(event.target.value); setValidationError('') }} placeholder="https://selu.example.com" /><Button size="sm" onClick={() => { setOrigin(settings.current_origin); setValidationError('') }}>{copy.useCurrent}</Button></div></Field>
      <div><SwitchRow title={copy.automatic} description={copy.automaticHint} checked={automatic} onChange={setAutomatic} /><SwitchRow title={copy.push} description={copy.pushHint} checked={push} onChange={setPush} icon={<Smartphone />} /><SwitchRow title={copy.telemetry} description={copy.telemetryHint} checked={telemetry} onChange={setTelemetry} /></div>
    </div>
  </ManagementSheet>
}

function isValidPublicOrigin(value: string) {
...
function SwitchRow({ title, description, checked, onChange, icon }: { title: string; description: string; checked: boolean; onChange: (next: boolean) => void; icon?: ReactNode }) { return <div className="final-switch-row"><span>{icon}{<strong>{title}</strong>}<small>{description}</small></span><button type="button" role="switch" className="final-switch" aria-checked={checked} aria-label={title} onClick={() => onChange(!checked)} /></div> }
