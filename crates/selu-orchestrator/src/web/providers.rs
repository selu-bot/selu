...
const PROVIDER_CATALOG: &[ProviderMeta] = &[
    ...
    ProviderMeta {
        ...
        category_key: "providers.category.cloud",
        subtitle_key: "providers.provider.grok.subtitle",
        requires_api_key: true,
        requires_base_url: false,
        api_key_label_key: "providers.field.api_key",
        base_url_label_key: "providers.field.base_url",
        api_key_placeholder_key: "providers.provider.grok.key_placeholder",
        base_url_placeholder_key: "providers.provider.grok.base_placeholder",
        base_url_hint_key: "providers.provider.grok.base_hint",
    ...
    ProviderMeta {
        ...
        subtitle_key: "providers.provider.pico.subtitle",
        requires_api_key: false,
        requires_base_url: true,
        api_key_label_key: "providers.provider.pico.key_label",
        base_url_label_key: "providers.provider.pico.url_label",
        api_key_placeholder_key: "providers.provider.pico.key_placeholder",
        base_url_placeholder_key: "providers.provider.pico.url_placeholder",
        base_url_hint_key: "providers.provider.pico.url_hint",
        default_base_url: "",
    },
];
...
async fn check_provider_access(
    ...
    match provider_id {
        ...
        "grok" => {
            ...
            if !resp.status().is_success() {
                return Err(anyhow::anyhow!("Grok returned HTTP {}", resp.status()));
            }
        }
        "bedrock" => {
        ...
        "pico" => {
            if base_url.trim().is_empty() {
                return Err(anyhow::anyhow!("Pico AI Server URL is required"));
            }
            let base = base_url.trim_end_matches('/').to_string();

            let resp = client.get(format!("{base}/v1/models")).send().await?;
            if !resp.status().is_success() {
                return Err(anyhow::anyhow!(
                    "Pico AI Server returned HTTP {}",
                    resp.status()
                ));
            }
        }
