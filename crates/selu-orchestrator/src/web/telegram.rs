...
pub async fn telegram_setup_page(
    ...
    if !external_origin.starts_with("https://") {
        return Redirect::to(&format!(
            "{}/pipes?error=Telegram+requires+HTTPS.+Access+Selu+via+HTTPS+or+save+an+https+web+address+in+System+Updates.",
            base_path
        )).into_response();
    }
...
pub async fn telegram_add_person(
    ...
    .await
    {
        Ok(_) => Redirect::to(&format!(
            "{}/pipes/telegram/{}?msg=pipes.person.added",
            base_path, config_id
        ))
        .into_response(),
        Err(e) if e.to_string().contains("UNIQUE") => Redirect::to(&format!(
            "{}/pipes/telegram/{}?error=tg.detail.person.exists",
            base_path, config_id
        ))
        .into_response(),
        Err(e) => {
            ...
            Redirect::to(&format!(
                "{}/pipes/telegram/{}?error=pipes.person.add.error",
                base_path, config_id
            ))
            .into_response()
        }
...
pub async fn telegram_check_webhook(
    ...
    match crate::telegram::adapter::get_webhook_info(&bot_token).await {
        ...
        Err(e) => {
            ...
            Html(
                r#"<div class="text-sm text-rosie animate-fadein" data-i18n="tg.troubleshoot.error">Could not check the Telegram connection right now.</div>"#.to_string()
            )
            .into_response()
        }
    }
...
pub async fn telegram_reregister_webhook(
    ...
    match crate::telegram::adapter::reregister_webhook(&state, &config_id, &external_origin).await {
        Ok(()) => {
            Html(
                r#"<div class="text-sm text-emerald-400 animate-fadein" data-i18n="tg.troubleshoot.registered">Telegram connection refreshed.</div>"#.to_string()
            ).into_response()
        }
        Err(e) => {
            error!("Failed to re-register webhook: {e}");
            Html(
                r#"<div class="text-sm text-rosie animate-fadein" data-i18n="tg.troubleshoot.refresh_error">Could not refresh the Telegram connection right now.</div>"#.to_string()
            )
            .into_response()
        }
    }
