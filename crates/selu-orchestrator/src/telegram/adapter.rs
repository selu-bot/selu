...
pub fn router() -> Router<AppState> {
    Router::new().route("/api/telegram/webhook/{config_id}", post(webhook_handler))
}
...
///
/// `external_origin` — if provided, overrides the saved public web address
...
pub async fn reregister_webhook(
    state: &AppState,
    config_id: &str,
    external_origin: &str,
) -> Result<()> {
...
async fn webhook_handler(
    ...
    // 6. Extract text/caption + image attachments
    let text = msg
        .text
        .as_ref()
        .or(msg.caption.as_ref())
        .cloned()
        .unwrap_or_default();
    let inbound_attachments = collect_inbound_attachments(&msg, &adapter.bot_token).await;
    if text.trim().is_empty() && inbound_attachments.is_empty() {
        debug!(config_id = %config_id, "Telegram webhook SKIPPED: no text/caption and no image");
        return StatusCode::OK;
    }
...
async fn collect_inbound_attachments(
    ...
    if let Some(photo) = msg.photo.last() {
        if let Some(data_base64) =
            fetch_telegram_file_as_base64(&http, bot_token, &photo.file_id).await
        {
            out.push(selu_core::types::InboundAttachment {
                filename: format!("telegram-photo-{}.jpg", msg.message_id),
                mime_type: "image/jpeg".to_string(),
                size_bytes: photo.file_size.and_then(|v| usize::try_from(v).ok()),
                download_url: None,
    ...
    if let Some(doc) = msg.document.as_ref() {
        ...
        if mime.starts_with("image/") {
            if let Some(data_base64) =
                fetch_telegram_file_as_base64(&http, bot_token, &doc.file_id).await
            {
                out.push(selu_core::types::InboundAttachment {
                    filename: doc
                        .file_name
                        .clone()
                        .unwrap_or_else(|| format!("telegram-image-{}", msg.message_id)),
                    mime_type: mime,
                    size_bytes: doc.file_size.and_then(|v| usize::try_from(v).ok()),
                    download_url: None,
                    data_base64: Some(data_base64),
                });
            }
...
async fn fetch_telegram_file_as_base64(
    http: &Client,
    bot_token: &str,
    file_id: &str,
) -> Option<String> {
    let file_path = fetch_telegram_file_path(http, bot_token, file_id).await?;
    let url = format!(
...
async fn fetch_telegram_file_path(http: &Client, bot_token: &str, file_id: &str) -> Option<String> {
    let url = format!("https://api.telegram.org/bot{}/getFile", bot_token.trim());
    let response = http
        .post(&url)
    ...
    let parsed = response.json::<TgGetFileResponse>().await.ok()?;
    if !parsed.ok {
        return None;
    }
    parsed.result.and_then(|r| r.file_path)
}
...
async fn set_webhook(bot_token: &str, webhook_url: &str) -> Result<()> {
    ...
    let tg_resp: TgGenericResponse =
        serde_json::from_str(&resp_body).context("Failed to parse Telegram setWebhook response")?;
    ...
    match resp {
        ...
        Ok(r) => {
            ...
            if body.contains("can't parse") {
                ...
                return send_telegram_plain(http, bot_token, chat_id, text, reply_to_message_id)
                    .await;
            }
...
/// Send a binary document via Telegram Bot API `sendDocument`.
async fn send_telegram_document(
    ...
    match http.post(&url).multipart(form).send().await {
        ...
        Ok(r) => {
            let status = r.status();
            let body = r.text().await.unwrap_or_default();
            error!(%status, %body, "Telegram rejected sendDocument");
            None
        }
        Err(e) => {
            error!("Failed to reach Telegram API (sendDocument): {e}");
            None
        }
    }
}

/// Send a binary image via Telegram Bot API `sendPhoto`.
async fn send_telegram_photo(
    ...
    let url = format!("https://api.telegram.org/bot{}/sendPhoto", bot_token.trim());
    ...
    match http.post(&url).multipart(form).send().await {
        ...
        Ok(r) => {
            let status = r.status();
            let body = r.text().await.unwrap_or_default();
            error!(%status, %body, "Telegram rejected sendPhoto");
            None
        }
        Err(e) => {
            error!("Failed to reach Telegram API (sendPhoto): {e}");
            None
        }
    }
}
...
pub async fn verify_bot_token(bot_token: &str) -> Result<String> {
    ...
    if !resp.status().is_success() {
        ...
        if status.as_u16() == 401 || status.as_u16() == 404 {
            anyhow::bail!(
                "Invalid bot token. Double-check the token you got from @BotFather — it should look like 123456:ABC-DEF1234ghIkl-zyx57W2v."
            );
        }
...
async fn dispatch_message(
    ...
    let inbound_envelope = selu_core::types::InboundEnvelope {
        ...
        metadata: Some(serde_json::json!({
            "message_guid": message_id.to_string(),
            "reply_to_ref": reply_to_message_id.map(|id| id.to_string()),
            "chat_ref": chat_id
        })),
    };
    ...
    let message_id_ref = inbound_envelope
        .metadata
        .as_ref()
    ...
    let reply_ref = inbound_envelope
        .metadata
        .as_ref()
    ...
    // Thread resolution
    let origin_ref = Some(message_id_ref.to_string());
    let reply_ref = reply_ref.map(|id| id.to_string());
    ...
    let sent_id = if !output.reply_text.is_empty() {
    ...
    for attachment in &output.attachments {
        if let Some(stored) = crate::agents::artifacts::get_for_user(
            &state.artifacts,
        ...
        .await
        {
            if stored.mime_type.starts_with("image/") {
                ...
                .await;
            } else if stored.mime_type == "application/pdf" {
                let _ = send_telegram_document(
                    &http,
                    &bot_token,
                    &chat_id,
                    &stored.filename,
...
impl crate::channels::ChannelSender for TelegramSender {
    async fn send_message(
        ...
        let reply_to_id = reply_to_guid.and_then(|s| s.parse::<i64>().ok());

        let msg_id = if !message.text.is_empty() {
            send_telegram_message(
                &self.http,
                &self.bot_token,
                &self.chat_id,
