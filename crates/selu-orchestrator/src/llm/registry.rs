...
use crate::llm::image_provider::{ImageApiFlavor, ImageProvider, OpenAiCompatibleImageProvider};
...
/// Loads an image provider from the database by provider ID and model ID.
///
/// Image providers are optional and currently use an OpenAI-compatible API.
pub async fn load_image_provider(
    db: &SqlitePool,
    provider_id: &str,
    model_id: &str,
    cred_store: &CredentialStore,
) -> Result<Arc<dyn ImageProvider>> {
    let row = sqlx::query!(
    ...
    .fetch_optional(db)
    .await?
    .ok_or_else(|| anyhow!("Image provider '{}' not found or inactive", provider_id))?;
    ...
    let api_key = match row.api_key_encrypted {
        Some(ref encrypted) if !encrypted.is_empty() => {
            ...
            String::from_utf8(plaintext_bytes)
                .map_err(|_| anyhow!("Image API key for '{}' is not valid UTF-8", provider_id))?
        }
        _ => String::new(),
    };
    ...
    let provider: Arc<dyn ImageProvider> = match provider_id {
        "openai" => {
            ...
            Arc::new(OpenAiCompatibleImageProvider::new(
                api_key,
                url,
                model_id,
                ImageApiFlavor::OpenAi,
            ))
        }
        "grok" => {
            let url = if base_url.is_empty() {
        ...
        other => {
            return Err(anyhow!(
                "Image provider '{}' is not supported for image operations",
                other
            ));
        }
    };
...
async fn load_provider_from_db(
    ...
    let provider: Arc<dyn LlmProvider> = match provider_id {
        ...
        "grok" => {
            let url = base_url.unwrap_or_else(|| "https://api.x.ai".into());
            let url = url.trim_end_matches('/').to_string();
            Arc::new(OpenAiProvider::grok(api_key, url, model_id))
        }
        "bedrock" => {
        ...
        "pico" => {
            let url = base_url.unwrap_or_else(|| "http://localhost:8090".into());
            let url = url.trim_end_matches('/').to_string();
            let effective_model = if model_id.is_empty() || model_id == "pico" {
                String::new() // Pico will use its default loaded model
            } else {
                model_id.to_string()
            };
            Arc::new(OpenAiProvider::pico(url, effective_model))
        }
