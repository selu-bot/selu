...
pub async fn get_effective_policy(
    ...
    let global = get_global_policy(db, agent_id, capability_id, tool_name).await?;
    if global.is_some() {
        return Ok(global);
    }

    // 3. Runtime defaults for built-in tools that should work out of the box
    // when no explicit policy row exists yet.
    if capability_id == BUILTIN_CAPABILITY_ID {
        match tool_name {
            // Delegation: allow cross-agent handoff to keep specialist workflows usable.
            BUILTIN_DELEGATE => return Ok(Some(ToolPolicy::Allow)),
            // Image tools: if the admin configured an image model (the only way these
            // tools appear in the spec), default to Allow rather than blocking silently.
...
pub async fn has_all_policies(
    ...
    required.push((
        BUILTIN_CAPABILITY_ID.to_string(),
        BUILTIN_IMAGE_GENERATE.to_string(),
    ));
    required.push((
        BUILTIN_CAPABILITY_ID.to_string(),
...
/// Tool name for the image_generate built-in (image generation).
pub const BUILTIN_IMAGE_GENERATE: &str = "image_generate";
