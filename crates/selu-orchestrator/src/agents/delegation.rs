...
pub fn tool_spec(
    ...
    ToolSpec {
        ...
        description: "Delegate a task to a specialised agent. Use this when the user's \
             ...
             For multi-step tasks, delegate one step at a time — e.g. first delegate \
             artifact creation, then delegate delivery or the next step separately."
            .to_string(),
        parameters: serde_json::json!({
            ...
            "properties": {
                ...
                "message": {
                    "type": "string",
                    "description": "The task for the specialist agent. Describe only the \
                                    step this agent should perform. Include any relevant \
                                    data or artifact references the specialist needs."
                }
            },
...
pub fn parse_args(args: &serde_json::Value) -> Result<(String, String)> {
    let agent_id = args
        ...
        .context("delegate_to_agent: missing 'agent_id' string (or legacy 'agent_name')")?
        .to_string();
...
mod tests {
    ...
    #[test]
    fn parse_args_accepts_legacy_agent_name() {
        let args = serde_json::json!({
            "agent_name": "weather",
            "message": "What's the weather in Berlin?"
        });
