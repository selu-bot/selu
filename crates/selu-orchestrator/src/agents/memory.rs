...
pub async fn search_for_context(
    ...
    if latest_message.trim().len() < 3 {
        return Ok(Vec::new());
    }

    search_memories(db, agent_id, user_id, latest_message, limit).await
}

pub fn remember_tool_spec() -> ToolSpec {
    ToolSpec {
        name: "memory_remember".to_string(),
        description: "Save a short memory that may help with future tasks for this user. \
             Use this for stable task context, recurring preferences for this agent, \
             and project details. Do not store secrets."
            .to_string(),
...
pub fn search_tool_spec() -> ToolSpec {
    ToolSpec {
        ...
        description: "Search stored memories using keyword relevance (BM25).".to_string(),
        parameters: serde_json::json!({
            "type": "object",
            "properties": {
                "query": {
...
pub fn list_tool_spec() -> ToolSpec {
    ToolSpec {
        ...
        description: "List recently stored memories.".to_string(),
        parameters: serde_json::json!({
            "type": "object",
            "properties": {
                "limit": {
...
pub async fn dispatch_forget(
    ...
    let deleted = delete_memory(db, agent_id, user_id, memory_id).await?;
...
pub async fn dispatch_search(
    ...
    let hits = search_memories(db, agent_id, user_id, query, limit).await?;
...
pub async fn dispatch_list(
    ...
    let entries = list_memories(db, agent_id, user_id, limit).await?;
...
mod tests {
    ...
    async fn memory_search_returns_relevant_entries() {
        ...
        let hits = search_memories(&db, "agent-1", "user-1", "backup friday", 5)
            .await
            .unwrap();
