...
impl CapabilityRunner {
    ...
    ///
    /// Pass `session_id = None` for shared (cross-session) containers.
    /// Shared containers omit `SELU_SESSION_ID` and set `SELU_SHARED=true`.
    ...
    pub async fn start(
        ...
        let container_name = if is_shared {
            ...
            format!("selu-cap-{}-{}", manifest.id, Uuid::new_v4().as_simple())
        };
        ...
        if let Some(sid) = session_id {
            env_vars.push(format!("SELU_SESSION_ID={}", sid));
        }
        if is_shared {
            env_vars.push("SELU_SHARED=true".to_string());
        }
    ...
    pub async fn update_policy(&self, token: &str, policy: ContainerEgressPolicy) {
        crate::capabilities::egress_proxy::register(
            &self.egress_registry,
            token.to_string(),
            policy,
