...
pub struct ContainerEgressPolicy {
    ...
    pub denied_hosts: Vec<String>,  // explicit deny entries always win
}
...
impl ContainerEgressPolicy {
    pub fn allows(&self, host: &str, port: u16) -> bool {
        if self
            .denied_hosts
            .iter()
            .any(|entry| host_entry_matches(entry, host, port))
        {
            return false;
        }
...
fn host_entry_matches(entry: &str, host: &str, port: u16) -> bool {
    ...
    if let Some(pattern) = entry.strip_prefix("*.") {
        if pattern.contains(':') {
            ...
            host.ends_with(&format!(".{}", base)) && entry.ends_with(&format!(":{}", port))
        } else {
...
fn parse_host_port(host_port: &str, default_port: u16) -> (String, u16) {
    ...
    if host_port.starts_with('[') {
        if let Some(close) = host_port.find(']') {
            let host = host_port[1..close].to_ascii_lowercase();
    ...
    match host_port.rsplit_once(':') {
        ...
        None => (host_port.to_ascii_lowercase(), default_port),
    }
}
...
mod tests {
    ...
    #[test]
    fn test_deny_overrides_any() {
        let mut p = policy(NetworkMode::Any, &[]);
        p.denied_hosts = vec!["pypi.org:443".into()];
        assert!(!p.allows("pypi.org", 443));
        assert!(p.allows("pypi.org", 80));
    }
