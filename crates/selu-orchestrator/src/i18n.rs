...
static TRANSLATIONS: LazyLock<HashMap<&'static str, HashMap<&'static str, &'static str>>> =
    LazyLock::new(|| {
        ...
        en.insert("chat.tool.using", "Using {tool}...");
        en.insert("chat.tool.preparing", "Preparing {tool}...");
        en.insert("chat.tool.delegating", "Asking {agent}...");
        en.insert(
            "chat.tool.delegating_progress",
            "Still working with {agent}...",
        );
        en.insert("chat.thinking", "Thinking...");
        en.insert("artifact.generated_files", "Generated files:");
        en.insert("chat.image_only_sent", "[Image sent]");
        en.insert("chat.image_attached_note", "[Image attached]");
        en.insert(
            "error.image_processing_unavailable",
            "I couldn't process the image just now. Please try again. If it still doesn't work, describe the image briefly and I'll help right away.",
        );
        en.insert(
            "image.model_not_configured",
            "Image creation isn't enabled yet. Ask your admin to set an image model in Settings.",
        );
        en.insert(
            "image.intent_ambiguous",
            "Do you want me to edit the image you sent, or create a brand new one?",
        );
        en.insert(
            "image.edit_requires_image",
            "Please send an image first, then tell me how you'd like it edited.",
        );
        en.insert("image.generated_ready", "Done. I created an image for you.");
        ...
        de.insert("chat.tool.using", "Nutze {tool}...");
        de.insert("chat.tool.preparing", "Bereite {tool} vor...");
        de.insert("chat.tool.delegating", "Frage {agent}...");
        de.insert(
            "chat.tool.delegating_progress",
            "Arbeite weiter mit {agent}...",
        );
        de.insert("chat.thinking", "Denkt nach...");
        de.insert("artifact.generated_files", "Erstellte Dateien:");
        de.insert("chat.image_only_sent", "[Bild gesendet]");
        de.insert("chat.image_attached_note", "[Bild angehängt]");
        de.insert(
            "error.image_processing_unavailable",
            "Ich konnte das Bild gerade nicht verarbeiten. Versuch es bitte nochmal. Wenn es weiter nicht klappt, beschreib das Bild kurz und ich helfe dir direkt.",
        );
        de.insert(
            "image.model_not_configured",
            "Die Bilderstellung ist noch nicht aktiviert. Bitte den Admin, in den Einstellungen ein Bildmodell festzulegen.",
        );
        de.insert(
            "image.intent_ambiguous",
            "Möchtest du, dass ich das gesendete Bild bearbeite oder ein ganz neues erstelle?",
        );
        de.insert(
            "image.edit_requires_image",
            "Bitte sende zuerst ein Bild und sag mir dann, wie ich es bearbeiten soll.",
        );
        de.insert(
            "image.generated_ready",
            "Fertig. Ich habe ein Bild für dich erstellt.",
        );
        de.insert(
            "image.edited_ready",
            "Fertig. Ich habe dein Bild bearbeitet.",
        );
...
/// Look up a translation and replace `{agent}` with the provided agent name.
pub fn t_with_agent(lang: &str, key: &str, agent_name: &str) -> String {
    let template = t(lang, key);
    template.replace("{agent}", agent_name)
}
