...
/// per-instance key stored in the `PAP_ENCRYPTION_KEY` env var (base64, 32 bytes).
///
...
pub fn parse_key(base64_key: &str) -> Result<[u8; 32]> {
    let raw = B64.decode(base64_key.trim())
        .context("PAP_ENCRYPTION_KEY is not valid base64")?;
    raw.try_into()
        .map_err(|_| anyhow!("PAP_ENCRYPTION_KEY must decode to exactly 32 bytes"))
}
