...
pub const TOOLS_NETWORK: &str = "pap-tools-net";

/// Name of the internal Docker bridge network for environment-class capabilities
pub const ENVS_NETWORK: &str = "pap-envs-net";
...
impl CapabilityRunner {
    ...
    pub async fn start(
        ...
        let env_vars = vec![
            ...
            format!("http_proxy={}", proxy_url),
            format!("https_proxy={}", proxy_url),
            format!("PAP_SESSION_ID={}", session_id),
            format!("PAP_CAPABILITY_ID={}", manifest.id),
        ];
...
async fn wait_for_grpc_ready(grpc_port: u16, timeout_secs: u64) -> Result<()> {
    ...
    use crate::capabilities::grpc::pap_capability::capability_client::CapabilityClient;
    use crate::capabilities::grpc::pap_capability::HealthRequest;
