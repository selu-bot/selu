...
///   HTTP_PROXY=http://pap:<token>@host.docker.internal:8888
///
/// The proxy:
...
async fn handle_connection(
    ...
    // Format: "Proxy-Authorization: Basic <base64(pap:token)>"
...
/// We expect the credentials to be `pap:<token>`, so we decode and extract the token part.
    ...
    for line in request.lines() {
        ...
        if lower.starts_with("proxy-authorization:") {
            ...
            if let Some(encoded) = value.strip_prefix("Basic ").or_else(|| value.strip_prefix("basic ")) {
                ...
                // Format is "pap:<token>" — extract the token after the colon
...
mod tests {
    ...
    fn test_extract_proxy_auth_token() {
        // "pap:my-secret-token" in base64 is "cGFwOm15LXNlY3JldC10b2tlbg=="
        let request = "CONNECT example.com:443 HTTP/1.1\r\nProxy-Authorization: Basic cGFwOm15LXNlY3JldC10b2tlbg==\r\n\r\n";
