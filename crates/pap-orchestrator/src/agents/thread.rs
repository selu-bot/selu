...
/// thread) and `last_reply_guid` (PAP's last outbound reply in the thread).
/// This enables iMessage reply-to chains: when a user replies to PAP's
...
/// Store PAP's outbound reply GUID on the thread so future incoming
